...
contains
  ...
  pure function denominators(n)
    ...
    integer(kind=ik), allocatable :: denominators(:)
    integer(kind=ik) :: i
    denominators = [integer(kind=ik) ::]
    do i = 1, n
  ...
  pure function num_tiles(n)
    ...
    !
    integer(kind=ik), intent(in) :: n
    integer(kind=ik) :: num_tiles(2)
    integer(kind=ik), allocatable :: denoms(:)
    integer(kind=ik), allocatable :: dim1(:), dim2(:)
    integer(kind=ik) :: i, j, n1, n2
    ...
    dim1 = [integer(kind=ik) ::]
    dim2 = [integer(kind=ik) ::]
    do j = 1, size(denoms)
      do i = 1, size(denoms)
  ...
  pure function tile_indices_1d(dims, i, n) result(indices)
    ...
    integer(kind=ik), intent(in) :: dims, i, n
    integer(kind=ik) :: indices(2)
  ...
  pure function tile_indices_2d(dims) result(indices)
    ...
    integer(kind=ik) :: indices(4)
    integer(kind=ik) :: tiles(2), tiles_ij(2)
  ...
  pure function tile_n2ij(n) result(ij)
    ...
    !
    integer(kind=ik), intent(in) :: n
    integer(kind=ik) :: ij(2), i, j, tiles(2)
    if (n == 0) then
      ij = 0
    else
      tiles = num_tiles(num_images())
  ...
  pure function tile_ij2n(ij) result(n)
    ...
    !   * tile_ij2n([3, 2]) = 6
    !
    integer(kind=ik), intent(in) :: ij(2)
    integer(kind=ik) :: n, tiles(2)
    if (any(ij == 0)) then
      n = 0
    else
      tiles = num_tiles(num_images())
  ...
  pure function tile_neighbors_2d(periodic) result(neighbors)
    ...
    integer(kind=ik) :: neighbors(4)
    integer(kind=ik) :: tiles(2), tiles_ij(2), itile, jtile
    integer(kind=ik) :: left, right, down, up
    integer(kind=ik) :: ij_left(2), ij_right(2), ij_down(2), ij_up(2)
  ...
  !   real(kind=rk), allocatable, intent(in out) :: a(:,:)
  !   integer(kind=ik), intent(in) :: indices(4)
  !   real(kind=rk), allocatable :: halo(:,:)[:]
  !   integer(kind=ik) :: tiles(2), neighbors(4)
  !   integer(kind=ik) :: is, ie, js, je
  !   integer(kind=ik) :: halo_size
  !
  ...
  !   real(kind=rk), allocatable :: halo(:,:)[:]
  !   integer(kind=ik) :: tiles(2), neighbors(4)
  !   integer(kind=ik) :: is, ie, js, je, di, dj
  !   integer(kind=ik) :: halo_size
  !
  !   if (.not. allocated(a)) then
  !     error stop 'Error in update_halo: input array not allocated.'
  ...
  !   ! sync all
  !
  !   ! write(*,*) shape(a), is, ie, js, je
  !
  !
  !   ! copy data into coarray buffer
  !   halo(1:dj,1)[neighbors(1)] = a(1,1:dj) ! send left
  !   halo(1:dj,2)[neighbors(2)] = a(di,1:dj) ! send right
  !   halo(1:di,3)[neighbors(3)] = a(1:di,1) ! send down
  !   halo(1:di,4)[neighbors(4)] = a(1:di,dj) ! send up
  !
  !   sync images(neighbors) !TODO currently fails with OpenCoarrays-2.2.0
  !   ! sync all
  !
  !   ! copy from halo buffer into array
  !   a(0,1:dj) = halo(1:dj,2) ! from left
  !   a(di+1,1:dj) = halo(1:dj,1) ! from right
  !   a(1:di,0) = halo(1:di,4) ! from down
  !   a(1:di,dj+1) = halo(1:di,3) ! from up
  !   sync all
  !   deallocate(halo)
  ! end subroutine sync_edges
