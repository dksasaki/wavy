...
pure function advectUpwind1stOrder1dRank1(f,u,dx) result(dfdt)
  ...
  do concurrent(i = 2:idm-1,m = 1:mdm)
    ...
    dfdt(m,i) = - 0.5_rk*((u(m,i+1)+abs(u(m,i+1)))*f(m,i)  &
...
endfunction advectUpwind1stOrder1dRank1
!-------------------------------------------------------------------------------
...
pure function advectUpwind1stOrder1dRank2(f,u,dx) result(dfdt)
  ...
  do concurrent(i = 2:idm-1,m = 1:mdm,n = 1:ndm)
    ...
    dfdt(m,n,i) = - 0.5_rk*((u(m,n,i+1)+abs(u(m,n,i+1)))*f(m,n,i)  &
...
endfunction advectUpwind1stOrder1dRank2
!-------------------------------------------------------------------------------
...
pure function advectCentered2ndOrder1dRank0(f,u,dx) result(dfdt)
  ...
  do concurrent(i = 2:idm-1)
    ...
    dfdt(i) = - 0.5_rk*(u(i+1)*(f(i)+f(i+1))&
                      - u(i)*(f(i-1)+f(i))) &
                      / dx(i)

  enddo

endfunction advectCentered2ndOrder1dRank0
!-------------------------------------------------------------------------------
...
pure function advectCentered2ndOrder1dRank1(f,u,dx) result(dfdt)
  ...
  do concurrent(i = 2:idm-1,m = 1:mdm)
    ...
    dfdt(m,i) = - 0.5_rk*(u(m,i+1)*(f(m,i)+f(m,i+1))&
                        - u(m,i)*(f(m,i-1)+f(m,i))) &
                        / dx(i)

  enddo

endfunction advectCentered2ndOrder1dRank1
!-------------------------------------------------------------------------------
...
pure function advectCentered2ndOrder2dRank0(f,u,v,dx,dy) result(dfdt)
  ...
  do concurrent(i = 2:idm-1,j = 2:jdm-1)
    ...
    dfdt(i,j) = - 0.25_rk*(u(i+1,j)*(dy(i,j)+dy(i+1,j))*(f(i,j)+f(i+1,j)) &
...
endfunction advectCentered2ndOrder2dRank0
!-------------------------------------------------------------------------------
